package org.apiphany.auth.oauth2.server;
	...
	static class TokenHandler implements HttpHandler {
		...
		public void handle(final HttpExchange exchange) throws IOException {
			...
			try (InputStream is = exchange.getRequestBody()) {
				String body = Strings.toString(is, StandardCharsets.UTF_8, DEFAULT_READ_BUFFER_SIZE);
				Map<String, String> params = RequestParameters.from(body);

				String clientId = params.get("client_id");
				boolean isAuthorized = AuthorizationGrantType.CLIENT_CREDENTIALS.matches(params.get("grant_type"))
						&& Objects.equals(server.clientId, clientId)
						&& Objects.equals(server.clientSecret, params.get("client_secret"));
				...
				token.setTokenType(HeaderValues.BEARER);
				token.setExpiresIn(getExpiresIn(params.get("expires_in")).toSeconds());
