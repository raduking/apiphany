...
- Fixed ephemeral client leak on exception path with `bleedExceptions=true`.
...
- Added `SSLHttpExchangeClient` decorator exchange client for SSL configuration and capability validation.
...
- Added `ApiMethod.Value.UNDEFINED` to be used for `ApiMethod.UNDEFINED` instead of `null`.
...
- Added `OAuth2TokenProviderRegistry.getProvider(String, Function)` to control missing provider behavior.
...
- Upgraded Micrometer to `1.15.10`.
...
- Upgraded `morphix-all` to `1.0.25`.
...
- Added `ContentEncoding.DEFLATE.decode` implementation using the `Deflate` class. 
- Added generic `ContentEncoding.decode(Object, Function)` static method to uniformly handle decoding.
...
- Added `AbstractHttpExchangeClient.isContentJson(ApiRequest)` method that returns true if the requests' content type is `application/json`.
